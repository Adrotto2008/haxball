...
async function createRoom() {
  ...
  channel
    ...
    .on('broadcast',{event:'input'}, msg => {
      const d=msg.payload;
      remoteInputs[d.id] = {up:!!(d.b&1), dn:!!(d.b&2), lt:!!(d.b&4), rt:!!(d.b&8), kick:!!(d.b&16)};
    })
    .on('broadcast',{event:'ping'}, msg => {
      pingMs = ~~((Date.now()-msg.payload.t)/2);
...
async function joinRoom(code) {
  ...
  channel
    ...
    .on('broadcast',{event:'team_update'}, msg => {
      const r=pmRoster.find(x=>x.id===msg.payload.id);
      if(r) {
        r.team=msg.payload.team;
    ...
    .on('broadcast',{event:'meta'}, msg => {
      const m=msg.payload;
      score=m.s; timeLeft=m.t; updateHUD();
      if(m.g && !gameOver) endGame();
    })
    .on('broadcast',{event:'chat'}, msg => { pushChatMsg(msg.payload, false); })
...
// ping: il guest manda un timestamp ogni 2s, l'host calcola la latenza
setInterval(()=>{ if(netMode==='guest'&&channel) channel.send({type:'broadcast',event:'ping',payload:{t:Date.now()}}); }, 2000);
