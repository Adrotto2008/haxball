...
function applyRemoteState() {
  ...
  for(const sp of s.ps) {
    ...
    p.team=sp.team; // aggiorna team per cambio in-game
  }
...
function sendChatMsg(text) {
  ...
  // in allenamento: solo locale
  if(!channel) { pushChatMsg(msg, true); return; }
...
function adminMoveTeamIngame(pid, team) {
  ...
  if(p) {
    p.team = team;
    if(team === -1) {
      // sposta spettatore fuori dal campo (posizione parcheggio)
      p.x = -9999; p.y = -9999; p.vx=0; p.vy=0;
    } else {
      // respawn nella metà campo corretta
      p.x = team===0 ? W*0.25 : W*0.75;
      p.y = H/2 + (Math.random()-0.5)*80;
      p.vx=0; p.vy=0;
    }
  }
  channel.send({type:'broadcast', event:'team_change', payload:{pid, team, roster:pmRoster}});
  broadcastState();
  if($('game-menu').classList.contains('open')) renderPmRoster();
}
...
function toggleAfk() {
  ...
  if(isAfk) {
    ...
    // torna nel team precedente (salvato nel roster)
    const r = pmRoster.find(x=>x.id===myPlayerId);
    if(r && r.team === -1) {
      r.team = r._prevTeam ?? 0;
    }
  } else {
    afkPlayers.add(myPlayerId);
    const r = pmRoster.find(x=>x.id===myPlayerId);
    if(r) { r._prevTeam = r.team; r.team = -1; }
    // sposta il proprio player fuori campo
    const p = players.find(x=>x.id===myPlayerId);
    if(p) { p.x=-9999; p.y=-9999; p.vx=0; p.vy=0; }
  }
  if(channel) channel.send({type:'broadcast', event:'afk', payload:{pid:myPlayerId, afk:!isAfk}});
  if($('game-menu').classList.contains('open')) renderPmRoster();
  const msg = {pid:'system',name:'Sistema',text: !isAfk ? '👻 Sei ora AFK (spettatore)' : '✅ Sei tornato in gioco', ts:Date.now()};
  pushChatMsg(msg, true);
}
...
function setSkin(skinVal) {
  ...
  if(channel) channel.send({type:'broadcast', event:'skin', payload:{pid:myPlayerId, skin:mySkin}});
}
