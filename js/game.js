...
function update(dt) {
  ...
  if(timeLeft>0) {
    secondAccum += dt;
    if(secondAccum>=1000) {
      secondAccum-=1000; timeLeft--; updateHUD();
      if(timeLeft===0){endGame();return;}
      if(netMode==='host' && timeLeft%5===0) broadcastMeta(); // sync timer ogni 5s
    }
  }
  if(netMode==='guest') {
...
function goal(team) {
  ...
  if(netMode==='host') { broadcastState(); broadcastMeta(); }
}
function endGame() {
  gameOver=true;
...
function startGame(mode, roster) {
  ...
  if(mode==='host') broadcastMeta(); // sync iniziale punteggio/timer al guest
}
